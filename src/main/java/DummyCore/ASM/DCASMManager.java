...
public class DCASMManager implements IClassTransformer {
    ...
    public static final String NetHandlerPlayClient_MethodHandleSpawnMobDesc =
            "(Lnet/minecraft/network/play/server/S0FPacketSpawnMob;)V";
    public static final String ObfNetHandlerPlayClient_MethodHandleSpawnMobDesc =
            "(Lnet/minecraft/network/play/server/S0FPacketSpawnMob;)V";
    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {
        ...
        if (basicClass != null) // If the class we are loading exists
        {
            if (name.equals(FMLClientHandlerName))
                ...
                if (name.endsWith(NetHandlerPlayClientName)
                        || name.endsWith(NotchNetHandlerPlayClientName)
                    ...
                    classReader.accept(
                            classNode,
                            0); // Giving our helper a parsed list of commands without any code modifications.
                    if (classNode.invisibleAnnotations != null
                            && classNode.invisibleAnnotations.size()
                                    > 0) // If we have some invisible annotations on our class.
                    {
                                ...
                                ++i) // Looking through all annotations presented on the class
                        {
                            ...
                            if (node.desc.equalsIgnoreCase(
                                    "LDummyCore/Utils/DCASMCheck;")) // If the given annotation is a DummyCore
                            // annotation, that sygnals, that the class needs
                            // to be checked.
                            {
                                checkClass = true;
    ...
    public byte[] handleNetHandlerPlayClientClass(String name, String transformedName, byte[] basicClass) {
        int obfuscationType =
                name.endsWith(NetHandlerPlayClientName) ? 0 : name.endsWith(ObfNetHandlerPlayClientName) ? 1 : 2;
        String S0FPacketSpawnMob = obfuscationType == 0 || obfuscationType == 1
                ? "net/minecraft/network/play/server/S0FPacketSpawnMob"
                : "fz";
        String EntityLivingBase = obfuscationType == 0
        ...
        if (CustomTXTConfig.mappings.containsKey("fixS0FSpawnMobPacketCrash")
                && CustomTXTConfig.mappings.get("fixS0FSpawnMobPacketCrash").contains("true")) {
            ...
            try {
                ...
                cr.accept(
                        cn,
                        ClassReader
                                .EXPAND_FRAMES); // <---- giving cn the instruction list. 0 is for flags, we want none.
                            ...
                            for (int j = 0; j < method.instructions.size(); ++j) {
                                ...
                                if (ain instanceof TypeInsnNode && ain.getOpcode() == Opcodes.CHECKCAST) {
                                    if (method.instructions.size() > j + 1) {
                                        ...
                                        if (ain1 instanceof VarInsnNode
                                                && ain1.getOpcode() == Opcodes.ASTORE
                ...
                ClassWriter cw = new ClassWriter(
                        ClassWriter.COMPUTE_MAXS); // <---- Creating the way we are writing Instructions into bytecode.
                // COMPUTE_MAXS will automatically calculate STACK size and LOCAL count
                // for each method.
    ...
    public byte[] handleForgeClientHandlerClass(String name, String transformedName, byte[] basicClass) {
        ...
        if (!CustomTXTConfig.mappings.containsKey("insertDCCallInTextureLoader")
                || !CustomTXTConfig.mappings.get("insertDCCallInTextureLoader").equals("true")) return basicClass;
        ...
        try {
            ...
            cr.accept(
                    cn,
                    ClassReader.EXPAND_FRAMES); // <---- giving cn the instruction list. 0 is for flags, we want none.
            ...
            ClassWriter cw = new ClassWriter(
                    ClassWriter.COMPUTE_MAXS); // <---- Creating the way we are writing Instructions into bytecode.
            // COMPUTE_MAXS will automatically calculate STACK size and LOCAL count for
            // each method.
            cn.accept(cw); // <---- Writing all modifications
