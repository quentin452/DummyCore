...
public class Core {
    ...
    private static void registerMod(Class<?> c, String name, boolean addCreativeTabs) {
        if (!isModRegistered(c)) {
            ...
            Notifier.notifyError(name + "[classPath:" + c
    ...
    private static void registerConfigurationFileForMod(Class<?> c, String path) {
        try {
            ...
            DCMod mod = getModFromClass(c);
            LoadingUtils.makeACrash(
                    "[DummyCore]Could not create a config file for mod " + mod + " - check your file system!", e, true);
        }
    }
     ...
     *
     */
    public static void registerModAbsolute(
            Class<?> c, String modname, String configPath, IDummyConfig config, boolean addCreativeTabs) {
