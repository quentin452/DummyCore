...
public class DummyDataUtils {
    ...
    public static void load(FMLServerAboutToStartEvent event) {
        String name =
                event.getServer().getFile(event.getServer().getFolderName()).getAbsolutePath();
        int length = 0;
    ...
    public static void loadCustomDataForMod(String modid, String dataName) {
        ...
        if (s == Side.CLIENT) {
            ...
            MiscUtils.registeredServerWorldData.put(
                    modid + "|" + dataName,
                    globalConfig.get(modid, dataName, "no data").getString());
            globalConfig.save();
            syncGlobalDataToClient(modid, dataName);
        }
    }
    ...
    public static void loadGlobalDataForMod(String modid) {
        ...
        if (s == Side.CLIENT) {
            ...
            MiscUtils.registeredServerWorldData.put(
                    modid + "|" + modid,
                    globalConfig.get(modid, modid, "no data").getString());
            globalConfig.save();
    ...
    public static void loadPlayerDataForMod(String playerName, String modid, String dataName) {
        ...
        if (s == Side.CLIENT) {
            ...
            MiscUtils.registeredServerData.put(
                    playerName + "_" + modid + "|" + dataName,
                    config.get(modid, dataName, "no data").getString());
            config.save();
            syncPlayerDataToClient(playerName, modid, dataName);
        }
    }
