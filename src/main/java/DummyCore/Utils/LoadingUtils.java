...
public class LoadingUtils {
    ...
    public static void makeACrash(String message, Class<?> thrower, Throwable t, boolean shutdown) {
        ...
        Notifier.notify(
                ...
                Notifier.error,
                "[DummyCoreLoader]",
                "Loading errors were detected at state " + Loader.instance().getLoaderState() + "!");
        Notifier.notify(
                shutdown ? Level.FATAL : Level.ERROR,
                "",
                Notifier.error,
        ...
        for (ModContainer mCon : tryDetermineModOffendorsFromClasses(tryDetermineOffendorsFromTrace(t)))
            Notifier.notify(
                    ...
                    Notifier.error,
                    "[DummyCoreLoader]",
                    "Likely involved mod: " + mCon.getName() + "[modid:" + mCon.getModId() + ",modClasspath:"
                            + (mCon.getMod() != null ? mCon.getMod().getClass() : "NULL{mod has no mod instance?}")
                            + "]");

        if (shutdown) FMLCommonHandler.instance().getSidedDelegate().haltGame(message, t);
    }
