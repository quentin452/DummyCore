...
public class GuiMainMenuNBT extends GuiMainMenu implements IMainMenu {
    ...
    public void confirmClicked(boolean yes, int id) {
        ...
        for (GuiButton btn : (List<GuiButton>) this.buttonList)
            if (btn.id == id && btn instanceof CustomButton && CustomButton.class.cast(btn).url != null) {
                if (yes) this.openURI(clickedURI);
    ...
    public void updateScreen() {
        for (int i = 0; i < objects.size(); ++i) objects.get(i).guiTick();
        ...
        if (menuType == 3 || menuType == 4) {
            ...
            if (!menuTextures.isEmpty()) {
                int maxTicks = menuTextures.get(textureIndex).getFirst()
                        + (menuType == 3 ? 0 : overlayTimeList.get(textureIndex));
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (menuType == 2) this.drawBackground(textureRepeat);
        if (menuType == 3) {
        ...
        if (menuType == 4) {
            ...
            if (tickTime >= maxTicks) {
                ...
                mc.renderEngine.bindTexture(menuTextures
                        .get(textureIndex + 1 >= menuTextures.size() ? 0 : textureIndex + 1)
                        .getSecond());
        ...
        for (AbstractGUIObject obj : objects) obj.drawOnScreen(mouseX, mouseY, partialTicks, zLevel + 10);
    ...
    public void drawBackground(int repeat) {
        ...
        tessellator.addVertexWithUV(
                0.0D, (double) this.height, 0.0D, 0.0D, (double) ((float) this.height / f + (float) repeat));
        tessellator.addVertexWithUV(
                (double) this.width, (double) this.height, 0.0D, (double) ((float) this.width / f), (double)
                        ((float) this.height / f + (float) repeat));
        tessellator.addVertexWithUV(
                (double) this.width, 0.0D, 0.0D, (double) ((float) this.width / f), (double) repeat);
    ...
    public void drawPanorama(int mouseX, int mouseY, float partialTicks) {
        ...
        for (int k = 0; k < b0 * b0; ++k) {
            ...
            GL11.glRotatef(
                    MathHelper.sin(((float) this.panoramaTimer + partialTicks) / 400.0F) * 25.0F + 20.0F,
                    1.0F,
                    0.0F,
                    0.0F);
            ...
            for (int l = 0; l < 6; ++l) {
                ...
                if (l == 1) GL11.glRotated(90, 0, 1, 0);

                if (l == 2) GL11.glRotated(180, 0, 1, 0);

                if (l == 3) GL11.glRotated(-90, 0, 1, 0);

                if (l == 4) GL11.glRotated(90, 1, 0, 0);

                if (l == 5) GL11.glRotated(-90, 1, 0, 0);
    ...
    public void initNBTInfo() {
        ...
        if (tag.hasKey("Images", 9)) {
            ...
            for (int i = 0; i < nbtLst.tagCount(); ++i) {
                ...
                if (image.gif) image.gifImage = new GIFImage(image.texture);

                int defaultX = tg.getInteger("XAlignment") == 0
                        ? 0
                        : tg.getInteger("XAlignment") == 1
                                ? this.width / 2
                                : tg.getInteger("XAlignment") == 2
                                        ? this.width
                                        : this.width / tg.getInteger("XAlignment");
                int defaultY = tg.getInteger("YAlignment") == 0
                        ? 0
                        : tg.getInteger("YAlignment") == 1
                                ? this.height / 2
                                : tg.getInteger("YAlignment") == 2
                                        ? this.height
                                        : this.height / tg.getInteger("YAlignment");
                ...
                if (tg.hasKey("Text", 9)) {
                    ...
                    for (int j = 0; j < texts.tagCount(); ++j) {
                        ...
                        String s = t.hasKey("Splashes")
                                ? createRandomSplash(new ResourceLocation(t.getString("Splashes")))
                                : t.getString("Text");
                        image.additionalText.add(
                                new Pair<Coord2D, String>(new Coord2D(t.getInteger("X"), t.getInteger("Y")), s));
                    }
                }
                objects.add(image);
            }
        ...
        if (tag.hasKey("Buttons", 9)) {
            ...
            for (int i = 0; i < nbtLst.tagCount(); ++i) {
                ...
                CustomButton button = new CustomButton(
                        ...
                        sizeX,
                        sizeY,
                        StatCollector.canTranslate(text) ? StatCollector.translateToLocal(text) : text);
                ...
                button.buttonYOffset = tg.hasKey("ButtonYUVOffset")
                        ? tg.getDouble("ButtonYUVOffset")
                        : (double) tg.getInteger("ButtonYOffset") / 256D;
                if (tg.hasKey("URL")) {
                    try {
    ...
    public String createRandomSplash(ResourceLocation splashesLoc) {
        ...
        try {
            ...
            if (!arraylist.isEmpty()) retStr = (String) arraylist.get(rand.nextInt(arraylist.size()));
    ...
    public void setupNBTInfo() {
        ...
        if (tag.hasKey("TextureRepeats")) this.textureRepeat = tag.getInteger("TextureRepeats");

        if (tag.hasKey("Textures", 9)) {
            NBTTagList nbtLst = tag.getTagList("Textures", 10);
            for (int i = 0; i < nbtLst.tagCount(); ++i) {
                NBTTagCompound tg = nbtLst.getCompoundTagAt(i);
                menuTextures.add(new Pair<Integer, ResourceLocation>(
                        tg.getInteger("FadeTime"), new ResourceLocation(tg.getString("Texture"))));
            ...
            if (tag.hasKey("Texture", 8)) {
                ...
                if (tag.getString("Texture").endsWith(".gif")) {
                    ...
                    for (int i = 0; i < gifImage.frames; ++i) overlayTimeList.add(tag.getInteger("FrameDelay"));

                    if (tag.hasKey("FramesDelay", 10)) {
                        NBTTagCompound framesDelay = tag.getCompoundTag("FramesDelay");
                        for (int i = 0; i < gifImage.frames; ++i)
                            if (framesDelay.hasKey(i + "", 3)) overlayTimeList.set(i, framesDelay.getInteger(i + ""));
                    }
                }
            } else {
                menuTexture = null;
        ...
        if (tag.hasKey("TextLeft", 9)) {
            ...
            for (int i = 0; i < nbtLst.tagCount(); ++i) {
                ...
                if (txt.equals("|SidedInfo|") && FMLCommonHandler.instance().getSidedDelegate() != null)
                    textLeft.addAll(
                            FMLCommonHandler.instance().getSidedDelegate().getAdditionalBrandingInformation());
                else if (!Strings.isNullOrEmpty(txt)) textLeft.add(txt);
            }
        ...
        if (tag.hasKey("TextRight", 9)) {
            ...
            for (int i = 0; i < nbtLst.tagCount(); ++i) {
                ...
                if (txt.equals("|SidedInfo|") && FMLCommonHandler.instance().getSidedDelegate() != null)
                    textRight.addAll(
                            FMLCommonHandler.instance().getSidedDelegate().getAdditionalBrandingInformation());
                else if (!Strings.isNullOrEmpty(txt)) textRight.add(txt);
            }
    ...
    public String parsePossibleLink(String s) {
        if (s.equals("|MC|")) s = Loader.instance().getMCVersionString();
        if (s.equals("|MCP|")) s = Loader.instance().getMCPVersionString();
        if (s.equals("|FML|")) s = "FML v" + Loader.instance().getFMLVersionString();
        if (s.equals("|Forge|")) s = MinecraftForge.getBrandingVersion();
        if (s.equals("|FMLBranding|"))
            s = Loader.instance().getFMLBrandingProperties().containsKey("fmlbranding")
                    ? Loader.instance().getFMLBrandingProperties().get("fmlbranding")
                    : "";
        if (s.equals("|Mods|")) {
        ...
        if (s.equals("|Copyright|")) s = "Copyright Mojang AB. Do not distribute!";
        return s;
    }
    ...
    public static class CustomImage extends GuiScreen implements AbstractGUIObject {
        ...
        public void drawOnScreen(int mouseX, int mouseY, float partialTicks, double zLevel) {
            ...
            if (!gif) {
                ...
                gifImage.drawOnScreen(
                        MathHelper.floor_double(frameTime + partialTicks) % gifImage.frames,
                        minU,
                        minV,
                        maxU,
                        maxV,
            ...
            if (!additionalText.isEmpty()) {
                for (Pair<Coord2D, String> p : additionalText) {
                    ...
                    if (splash) text = text.substring(8);
    ...
    public static class CustomButton extends GuiButton implements AbstractGUIObject {
        ...
        public void func_146113_a(SoundHandler sh) {
            sh.playSound(PositionedSoundRecord.func_147674_a(
                    sound != null ? sound : new ResourceLocation("gui.button.press"), 1.0F));
        }
