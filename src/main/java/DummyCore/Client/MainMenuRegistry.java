...
public class MainMenuRegistry {
    ...
    public static void registerNewGui(Class<? extends GuiScreen> menu) {
        if (IMainMenu.class.isAssignableFrom(menu)) {
            ...
            Notifier.notifyCustomMod(
                    "DummyCore",
                    "Attempting to register " + menu
                            + " as a main menu, but the registered object does not implements IMainMenu!");
        }
    }
    ...
    public static void initMenuConfigs() {
        try {
                ...
                if (detected.getName().endsWith(".dcmenu")) {
                    ...
                    if (fis != null) {
                        try {
                            ...
                            if (validateNBT(tag)) {
                                tagsToMenu.add(tag);
                                Notifier.notifySimple("[MainMenuRegistry]" + detected
                                        + " Is a valid main menu and has been send to parsing queue.");
                            } else {
                                Notifier.notifyError("[MainMenuRegistry]" + detected + " Is not a valid menu!");
                            }
                        } catch (IOException ioe) {
                            LoadingUtils.makeACrash(
                                    "[MainMenuRegistry]Couldn't read file " + detected, clazz, ioe, false);
                        } catch (UnsupportedCharsetException uce) {
                            LoadingUtils.makeACrash(
                                    "[MainMenuRegistry]UTF-8 is unsupported on your system(???)", clazz, uce, false);
                        } catch (NBTException nbte) {
                            LoadingUtils.makeACrash(
                                    "[MainMenuRegistry]" + detected + " Is not a valid menu!", clazz, nbte, false);
            ...
            LoadingUtils.makeACrash(
                    "[MainMenuRegistry]Couldn't read files in your config directory", clazz, ioe, false);
        }
    }
